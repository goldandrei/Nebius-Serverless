.PHONY: serve list-models up eval down dashboard sweep clean
...
eval:
	$(UVPY) -c "import json,sys; sys.path.insert(0,'.'); from src.eval_runner import run; r=run(backend='endpoint'); open('results/results.json','w').write(json.dumps(r,indent=2)); print('done — results/results.json')"
